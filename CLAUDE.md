...
**没有 .sln 文件**，直接操作 `TerminalMCP/TerminalMCP.csproj`。
...
4. 返回 `ReadResult`：`hwnd`、`title`、`total_lines`、`lines_read`、`text`（用 `\n` 拼接）
...
3. 若有基线 → 行级前缀比对：从第 0 行开始逐行比较 `previousLines[i] == currentLines[i]`
   - 在旧内容范围内未找到分叉 → 旧内容是当前内容的前缀，返回 `previousLines.Length..` 的追加行
   - 在旧内容范围内找到分叉 → 返回从分叉点到末尾的所有行
4. 更新 `_baselines` 为最新内容
5. 返回 `DiffResult`：`status` 为 `"init"` / `"new"` / `"no_change"`，`text` 为新内容，`new_line_count` 为新行数
